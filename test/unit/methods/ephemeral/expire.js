...
const method = require('../../../../lib/methods/ephemeral/expire')()
...
test('### Should expires a record ###', function (t) {
  ...
  // Mocks
  const expireMock = sandbox.mock(connector.client).expects('expire').once().withArgs('1', 60).yieldsAsync(null, 1)
  // Spies
  const cbSpy = sandbox.spy()

  // Function call
  method.call(connector, testModel, fakeInst, 60, cbSpy)
...
test('### Should returns an error ###', function (t) {
  ...
  // Mocks
  const expireMock = sandbox.mock(connector.client).expects('expire').once().withArgs('1', 60).yieldsAsync(err)
  // Spies
  const cbSpy = sandbox.spy()
